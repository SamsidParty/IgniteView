...
namespace WebFramework
{
    ...
    {
        //Why such a stupid way of storing this library you may ask?
        //Why not use resources?
        //Welp loading resources from libraries doesn't work on MAUI properly
        //See https://github.com/dotnet/maui/issues/7443
        //Despite some people saying it's "fixed", we still run into FileNotFoundExceptions
...
window.JSI_IsReady = true;
        ...
        /// <summary>
        /// Wraps The JavaScript In Eval.Call, So That It Can Be Run In A Global Context
        /// </summary>
        /// <param name="content"></param>
        /// <returns></returns>
        public static string WrapContent(string content)
        {
            var wrapStart = "eval.call(window, atob('";
            var wrapEnd = "'));";

            return wrapStart + Convert.ToBase64String(Encoding.UTF8.GetBytes(content)) + wrapEnd;
        }
    }
}
